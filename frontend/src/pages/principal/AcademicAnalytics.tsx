...
import { usePrincipalAnalytics } from '../../hooks/usePrincipal';

function AcademicAnalytics() {
  const { data, isLoading } = usePrincipalAnalytics();
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-[26px] border border-[#EFF2F6] bg-white p-6 shadow-sm">
        <p className="text-[10px] uppercase tracking-[0.28em] text-slate-500">Academic analytics</p>
        <h1 className="mt-3 text-2xl font-semibold text-slate-950">Performance Monitoring</h1>
        <div className="mt-6 grid gap-3 sm:grid-cols-3">
          {[
            ['Completion rate', `${data.completionRate}%`, '#639922'],
            ['Delay percentage', `${data.delayPercentage}%`, '#E24B4A'],
            ['Academic efficiency', `${data.academicEfficiency}%`, '#185FA5']
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        ...
        <section className="rounded-[26px] border border-[#EFF2F6] bg-white p-6 shadow-sm">
          <h2 className="text-lg font-semibold text-slate-950">Faculty Performance</h2>
          <div className="mt-5 h-72">
            <ResponsiveContainer>
