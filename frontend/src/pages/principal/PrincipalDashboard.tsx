...
import PrincipalTaskTable from '../../components/principal/PrincipalTaskTable';
import TaskDetailModal from '../../components/principal/TaskDetailModal';
import { useSocket } from '../../hooks/useSocket';
import { usePrincipalDashboard, usePrincipalDelayAlerts, usePrincipalTasks } from '../../hooks/usePrincipal';
...
function PrincipalTasks() {
  const [filter, setFilter] = useState<TaskStatus | 'ALL'>('ALL');
  const [selectedTask, setSelectedTask] = useState<Task | null>(null);
  const { data = [], isLoading } = usePrincipalTasks(filter);
    ...
    <>
      <section className="space-y-6">
        <div className="rounded-[26px] border border-[#EFF2F6] bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            <div>
              ...
              <h1 className="mt-3 text-2xl font-semibold text-slate-950">My Academic Tasks</h1>
              <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600">
                View task details, update status, upload attachments, and review task history.
              </p>
            </div>
        ...
        <div className="rounded-[26px] border border-[#EFF2F6] bg-white p-6 shadow-sm">
          ...
          <PrincipalTaskTable onView={setSelectedTask} tasks={data} />
        </div>
      </section>
...
function DelayAlerts() {
  const { data = [] } = usePrincipalDelayAlerts();
    ...
    <div className="space-y-6">
      <section className="rounded-[26px] border border-[#EFF2F6] bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <p className="text-[10px] uppercase tracking-[0.28em] text-slate-500">Academic Module</p>
            <h1 className="mt-3 text-2xl font-semibold text-slate-950">Principal Control Panel</h1>
            <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-600">
              Monitor teaching operations, academic task progress, delayed work, announcements, and real-time school activity.
            </p>
          </div>
          <NavLink className="rounded-[14px] bg-[#185FA5] px-5 py-3 text-sm font-semibold text-white" to="/principal/tasks">
            Review Tasks
          </NavLink>
        </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_0.8fr]">
        <section className="rounded-[26px] border border-[#EFF2F6] bg-white p-6 shadow-sm">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold text-slate-950">Academic Task Progress</h2>
      ...
      <main className="min-w-0 flex-1">
        ...
        <div className="h-full overflow-y-auto p-6">
          <Routes>
            ...
            <Route path="tasks" element={<PrincipalTasks />} />
