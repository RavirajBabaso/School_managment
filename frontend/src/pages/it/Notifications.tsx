...
import { useITNotifications } from '../../hooks/useIT';

function Notifications() {
  const queryClient = useQueryClient();
  const { data: notifications = [], isLoading } = useITNotifications();

  const markReadMutation = useMutation({
    mutationFn: (id: number) => itService.markITNotificationRead(id),
    onSuccess: () => void queryClient.invalidateQueries({ queryKey: ['it-notifications'] })
  });

  const markAllReadMutation = useMutation({
    mutationFn: () => itService.markAllITNotificationsRead(),
    onSuccess: () => void queryClient.invalidateQueries({ queryKey: ['it-notifications'] })
  });
    ...
    <div className="space-y-6">
      <div className="rounded-[26px] border border-[#EFF2F6] bg-white p-6 shadow-sm">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold text-slate-950">IT Notifications</h2>
          {unreadCount > 0 ? (
            <button
