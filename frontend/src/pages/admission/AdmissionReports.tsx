...
import { useAdmissionReports } from '../../hooks/useAdmission';
import * as admissionService from '../../services/admissionService';

function AdmissionReports() {
  const queryClient = useQueryClient();
  const { data = [], isLoading } = useAdmissionReports();
  const now = new Date();

  const generate = useMutation({
    mutationFn: (type: 'daily' | 'weekly' | 'monthly') => {
      if (type === 'daily') {
        return admissionService.generateAdmissionReport('daily', { date: now.toISOString() });
      }
      if (type === 'weekly') {
        return admissionService.generateAdmissionReport('weekly', { week: now.toISOString() });
      }
      return admissionService.generateAdmissionReport('monthly', {
        month: now.getMonth() + 1,
        year: now.getFullYear()
      });
    },
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['admission-reports'] })
  });
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-[26px] border border-[#EFF2F6] bg-white p-6 shadow-sm">
        ...
        <h1 className="mt-3 text-2xl font-semibold text-slate-950">Admission Reports</h1>
