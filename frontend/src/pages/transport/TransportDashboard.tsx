...
import TransportTaskTable from '../../components/transport/TransportTaskTable';
import TaskDetailModal from '../../components/transport/TaskDetailModal';
import { useSocket } from '../../hooks/useSocket';
import { useTransportDashboard, useTransportDelayAlerts, useTransportTasks } from '../../hooks/useTransport';
...
function TransportTasks() {
  ...
  const { data = [], isLoading } = useTransportTasks(filter);
      ...
      <section className="space-y-6">
        <div className="rounded-[26px] border border-[#EFF2F6] bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            <div>
              <p className="text-[10px] uppercase tracking-[0.28em] text-slate-500">Transport Task Monitoring</p>
              <h1 className="mt-3 text-2xl font-semibold text-slate-950">Transport Tasks</h1>
              <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600">
                Track transport operations, vehicle coordination, driver task management, route monitoring, and transportation-related task lifecycle.
              </p>
            </div>
        ...
        <div className="rounded-[26px] border border-[#EFF2F6] bg-white p-6 shadow-sm">
          ...
          <TransportTaskTable onView={setSelectedTask} tasks={data} />
        </div>
      </section>
...
function DelayAlerts() {
  const { data = [] } = useTransportDelayAlerts();
...
function TransportOverview() {
  const { data, isLoading } = useTransportDashboard();
  const summary = data ?? {
    completedTasks: 0,
    completionRate: 0,
    delayedTasks: 0,
    ...
    <div className="space-y-6">
      <section className="rounded-[26px] border border-[#EFF2F6] bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <p className="text-[10px] uppercase tracking-[0.28em] text-slate-500">Transport Department Module</p>
            <h1 className="mt-3 text-2xl font-semibold text-slate-950">Transport Control Panel</h1>
            <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-600">
              Manage transport operations, vehicle coordination, driver task management, route monitoring, and transportation-related task lifecycle management.
            </p>
          </div>
          <NavLink className="rounded-[14px] bg-[#185FA5] px-5 py-3 text-sm font-semibold text-white" to="/transport/tasks">
            Review Tasks
          </NavLink>
        </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_0.8fr]">
        <section className="rounded-[26px] border border-[#EFF2F6] bg-white p-6 shadow-sm">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold text-slate-950">Transport Task Progress</h2>
      ...
      <main className="min-w-0 flex-1">
        ...
        <div className="h-full overflow-y-auto p-6">
          <Routes>
            ...
            <Route path="dashboard" element={<TransportOverview />} />
