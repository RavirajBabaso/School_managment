...
import HRTaskTable from '../../components/hr/HRTaskTable';
import TaskDetailModal from '../../components/hr/TaskDetailModal';
import { useSocket } from '../../hooks/useSocket';
import { useHRAnalytics, useHRDashboard, useHRDelayAlerts, useHRTasks } from '../../hooks/useHR';
...
function HRTasks() {
  ...
  const { data = [], isLoading } = useHRTasks(filter);
      ...
      <section className="space-y-6">
        <div className="rounded-[26px] border border-[#EFF2F6] bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            <div>
              <p className="text-[10px] uppercase tracking-[0.28em] text-slate-500">HR Task Monitoring</p>
              <h1 className="mt-3 text-2xl font-semibold text-slate-950">HR Tasks</h1>
              <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600">
                Track staff tasks, employee coordination, performance monitoring, and task lifecycle management.
              </p>
            </div>
        ...
        <div className="rounded-[26px] border border-[#EFF2F6] bg-white p-6 shadow-sm">
          ...
          <HRTaskTable onView={setSelectedTask} tasks={data} />
        </div>
      </section>
    ...
    <div className="space-y-6">
      <section className="rounded-[26px] border border-[#EFF2F6] bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <p className="text-[10px] uppercase tracking-[0.28em] text-slate-500">HR Management Module</p>
            <h1 className="mt-3 text-2xl font-semibold text-slate-950">HR Control Panel</h1>
            <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-600">
              Manage staff lifecycle, employee coordination, HR operations, performance monitoring, task tracking, and communication.
            </p>
          </div>
          <NavLink className="rounded-[14px] bg-[#185FA5] px-5 py-3 text-sm font-semibold text-white" to="/hr/tasks">
            Review Tasks
          </NavLink>
        </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_0.8fr]">
        <section className="rounded-[26px] border border-[#EFF2F6] bg-white p-6 shadow-sm">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold text-slate-950">HR Task Progress</h2>
