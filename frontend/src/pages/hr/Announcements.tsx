...
import { useHRAnnouncements } from '../../hooks/useHR';

function Announcements() {
  const [query, setQuery] = useState('');
  const { data = [], isLoading } = useHRAnnouncements();
  ...
  return (
    <section className="rounded-[26px] border border-[#EFF2F6] bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <p className="text-[10px] uppercase tracking-[0.28em] text-slate-500">HR Communications</p>
          <h1 className="mt-3 text-2xl font-semibold text-slate-950">Announcements</h1>
          <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600">
            View school-wide, HR department, and urgent announcements.
          </p>
        </div>
        <input
