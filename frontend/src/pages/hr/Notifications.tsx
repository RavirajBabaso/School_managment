...
import { useHRNotifications } from '../../hooks/useHR';
import * as hrService from '../../services/hrService';

function Notifications() {
  const queryClient = useQueryClient();
  const { data = [], isLoading } = useHRNotifications();

  const markRead = useMutation({
    mutationFn: hrService.markHRNotificationRead,
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['hr-notifications'] })
  });

  const markAllRead = useMutation({
    mutationFn: hrService.markAllHRNotificationsRead,
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['hr-notifications'] })
  });
  ...
  return (
    <section className="rounded-[26px] border border-[#EFF2F6] bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-[10px] uppercase tracking-[0.28em] text-slate-500">HR Notifications</p>
