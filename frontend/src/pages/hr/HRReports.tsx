...
function HRReports() {
  ...
  const { data = [], isLoading } = useHRReports();
  const now = new Date();

  const generate = useMutation({
    mutationFn: (type: 'daily' | 'weekly' | 'monthly') => {
      if (type === 'daily') {
        return hrService.generateHRReport('daily', { date: now.toISOString() });
      }
      if (type === 'weekly') {
        return hrService.generateHRReport('weekly', { week: now.toISOString() });
      }
      return hrService.generateHRReport('monthly', {
        month: now.getMonth() + 1,
        year: now.getFullYear()
      });
    },
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['hr-reports'] })
  });
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-[26px] border border-[#EFF2F6] bg-white p-6 shadow-sm">
        ...
        <h1 className="mt-3 text-2xl font-semibold text-slate-950">HR Reports</h1>
