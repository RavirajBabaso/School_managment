...
import PurchaseTaskTable from '../../components/purchase/PurchaseTaskTable';
import TaskDetailModal from '../../components/purchase/TaskDetailModal';
import { useSocket } from '../../hooks/useSocket';
import { usePurchaseDashboard, usePurchaseTasks, usePurchaseDelayAlerts, usePurchaseAnalytics } from '../../hooks/usePurchase';
...
function PurchaseTasks() {
  const [filter, setFilter] = useState<TaskStatus | 'ALL'>('ALL');
  const [selectedTask, setSelectedTask] = useState<Task | null>(null);
  const { data = [], isLoading } = usePurchaseTasks(filter);
      ...
      <section className="space-y-6">
        <div className="rounded-[26px] border border-[#EFF2F6] bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            <div>
              <p className="text-[10px] uppercase tracking-[0.28em] text-slate-500">Purchase Task Monitoring</p>
              <h1 className="mt-3 text-2xl font-semibold text-slate-950">Purchase Tasks</h1>
              <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600">
                Track procurement tasks, vendor coordination, purchase orders, and delivery monitoring.
              </p>
            </div>
        ...
        <div className="rounded-[26px] border border-[#EFF2F6] bg-white p-6 shadow-sm">
          ...
          <PurchaseTaskTable onView={setSelectedTask} tasks={data} />
        </div>
      </section>
...
function PurchaseDelayAlerts() {
  const { data = [] } = usePurchaseDelayAlerts();
    ...
    <div className="space-y-6">
      <section className="rounded-[26px] border border-[#EFF2F6] bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <p className="text-[10px] uppercase tracking-[0.28em] text-slate-500">Purchase Management Module</p>
            <h1 className="mt-3 text-2xl font-semibold text-slate-950">Purchase Control Panel</h1>
            <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-600">
              Manage procurement operations, vendor coordination, purchase orders, inventory tracking, and supply chain management.
            </p>
          </div>
          <NavLink className="rounded-[14px] bg-[#185FA5] px-5 py-3 text-sm font-semibold text-white" to="/purchase/tasks">
            Review Tasks
          </NavLink>
        </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_0.8fr]">
        <section className="rounded-[26px] border border-[#EFF2F6] bg-white p-6 shadow-sm">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold text-slate-950">Procurement Progress</h2>
      ...
      <main className="min-w-0 flex-1">
        ...
        <div className="h-full overflow-y-auto p-6">
          <Routes>
            ...
            <Route path="dashboard" element={<PurchaseOverview />} />
            <Route path="tasks" element={<PurchaseTasks />} />
