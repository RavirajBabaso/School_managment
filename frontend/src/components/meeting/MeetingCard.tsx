  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-4 mb-3">
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-xs">{meeting.organizer}</span>
      </div>
      ...
      {hasResponded === 'ACCEPTED' && (
        <p className="text-xs text-green-600 italic">✓ Meeting accepted</p>
      )}
      {hasResponded === 'DECLINED' && (
        <p className="text-xs text-red-600 italic">✗ Meeting declined</p>
      )}
    </div>
  );
};
