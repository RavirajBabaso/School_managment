...
function ITTaskTable({ onView, tasks }: { onView: (task: Task) => void; tasks: Task[] }) {
  return (
            ...
            {tasks.map((task) => (
              <tr className="hover:bg-[#F8FAFC]" key={task.id}>
                ...
                <td className="px-4 py-4">
                  <span className={`rounded-full px-3 py-1 text-xs font-semibold ${tone(task.priority)}`}>{task.priority}</span>
                </td>
                ...
                <td className="px-4 py-4">
                  <span className={`rounded-full px-3 py-1 text-xs font-semibold ${tone(task.status)}`}>{task.status.replace('_', ' ')}</span>
                </td>
                <td className="px-4 py-4 text-[#185FA5]">{task.attachment_path ? 'View file' : '--'}</td>
                <td className="px-4 py-4">
                  <button
