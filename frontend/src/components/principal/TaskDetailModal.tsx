...
function TaskDetailModal({
  ...
  const mutation = useMutation({
    mutationFn: () => principalService.updatePrincipalTask(task!.id, { status, comment, attachment }),
    onSuccess: async () => {
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: ['principal-dashboard'] }),
        queryClient.invalidateQueries({ queryKey: ['principal-tasks'] }),
        queryClient.invalidateQueries({ queryKey: ['principal-analytics'] })
      ]);
      setComment('');
    ...
    <Modal bodyClassName="space-y-6" isOpen={isOpen} onClose={onClose} title="Task Details">
      <div className="rounded-[20px] border border-[#EFF2F6] bg-[#F8FAFC] p-5">
        ...
        <div className="mt-4 grid gap-3 text-sm sm:grid-cols-2">
          <p><span className="text-slate-500">Assigned by:</span> {task.assignedBy?.name ?? task.assignedByName ?? 'Academic Office'}</p>
