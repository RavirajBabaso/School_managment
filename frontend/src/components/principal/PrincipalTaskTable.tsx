          ...
          <tbody className="divide-y divide-[#EFF2F6]">
            {tasks.map((task) => (
              <tr className="hover:bg-[#F8FAFC]" key={task.id}>
                ...
                <td className="px-4 py-4 text-slate-600">{task.assignedBy?.name ?? task.assignedByName ?? 'Academic Office'}</td>
...
export default PrincipalTaskTable;
