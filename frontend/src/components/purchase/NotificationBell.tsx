...
function PurchaseNotificationBell() {
  ...
  return (
    <div className="relative" ref={containerRef}>
      <button
        ...
        className="relative flex h-10 w-10 items-center justify-center rounded-full border border-slate-300 bg-white text-slate-700 transition-all hover:bg-[#EEF4FF] hover:text-slate-950"
      ...
      >
        ...
        {unreadCount > 0 ? (
          <span className="absolute -right-1 -top-1 flex min-h-[18px] min-w-[18px] items-center justify-center rounded-full bg-[#185FA5] px-1 text-[10px] font-bold text-white">
        ...
        <div className="absolute right-0 top-12 z-30 w-[340px] overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-[0_20px_50px_rgba(15,23,42,0.14)]">
          <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
            <div>
              ...
              <p className="mt-1 text-[11px] text-slate-500">Latest updates</p>
            </div>
          </div>
              ...
              latestNotifications.map((notification: Notification) => (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <p className="text-[13px] font-medium leading-5 text-slate-950">{notification.message}</p>
                    {!notification.is_read && <span className="mt-1 h-2.5 w-2.5 shrink-0 rounded-full bg-[#185FA5]" />}
                  </div>
                  <p className="mt-2 text-[11px] text-slate-500">{formatTimestamp(notification.created_at)}</p>
                </div>
              ))
            ) : (
...
export default PurchaseNotificationBell;
